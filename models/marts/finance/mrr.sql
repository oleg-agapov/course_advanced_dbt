...
monthly_subscriptions AS (
    SELECT
        ...
        plan_name,
        pricing,
        DATE({{ truncate_monthly('starts_at') }}) AS start_month,
        DATE({{ truncate_monthly('ends_at') }}) AS end_month
    FROM
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATE(DATEADD('month', 1, {{ truncate_monthly('CURRENT_DATE') }}))
            ELSE end_month
        END AS end_month
    FROM
...
final AS (
    ...
    WHERE
        date_month <= {{ truncate_monthly('CURRENT_DATE') }}
)
